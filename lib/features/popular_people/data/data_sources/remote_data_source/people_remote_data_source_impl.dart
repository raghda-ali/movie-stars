...
class PeopleRemoteDataSourceImpl implements PeopleRemoteDataSource {
  ...
  @override
  Future<PersonResponseModel> getPopularPeople({required int page}) async {
    try {
      final response = await dioService.get(
        url: 'popular',
        queryParameters: {'api_key': RemoteUrls.apiKey, 'page': page},
      );
      return PersonResponseModel.fromJson(response.data);
    } catch (e) {
      rethrow;
    }
  }
  ...
  Future<PersonModel> getPersonDetails({required int personId}) async {
    final response = await dioService.get(
      url: '$personId',
      queryParameters: {'api_key': RemoteUrls.apiKey},
    );
    return PersonModel.fromJson(response.data);
  }

  @override
  Future<PersonImagesResponseEntity> getPersonImages({
