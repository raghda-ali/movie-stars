    ...
    return Scaffold(
      body: BlocConsumer<PopularPeopleBloc, PopularPeopleState>(
        listener: (context, state) {
          if (state.personBasicInfoStatus == RequestStatus.error) {
            context.showSnackBar(
              context: context,
              message:
                  state.personBasicInfoError ?? "Failed to load person info",
            );
          } else if (state.personImagesStatus == RequestStatus.error) {
            context.showSnackBar(
              context: context,
              message:
                  state.personImagesError ?? 'Failed to load person images',
            );
          }
        },
                  ...
                  sliver: SliverList(
                    delegate: SliverChildListDelegate([
                      if (state.personBasicInfoStatus == RequestStatus.error)
                        Center(
                          child: Text(
                            "Failed to load person details, try again later",
                          ),
                        )
                      else ...[
                        Center(
